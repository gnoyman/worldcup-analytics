 ...
 *
 *   FOOTBALL_DATA_PROVIDER=football-data-org → FootballDataOrgProvider (football-data.org v4)
...
export { FootballDataOrgProvider, FDO_CACHE_MATCHES, FDO_CACHE_STANDINGS, FDO_CACHE_SCORERS } from "./footballDataOrgProvider";
export {
...
export function getProvider(): FootballProvider {
  ...
  if (explicitProvider === "football-data-org") {
    const apiKey = process.env.FOOTBALL_DATA_ORG_KEY;
    if (!apiKey) throw new Error("[football] FOOTBALL_DATA_ORG_KEY is not set (required for football-data-org provider).");
...
export function getProviderName(): string {
  const explicitProvider = process.env.FOOTBALL_DATA_PROVIDER?.toLowerCase();
  if (explicitProvider === "football-data-org") return "FootballDataOrgProvider";
